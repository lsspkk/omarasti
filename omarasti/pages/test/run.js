...
const Test2 = ({ mapUrl }) => {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => setAngle(angle + 5), 100)
  ...
  return (
    <Layout map='true' menu={<div />}>
      ...
      {what === 'touch' && <TouchMarkerPanel touchMarker={() => ''} track={track} markerNumber={1} />}
      {what === 'see' && <SeeMarkerPanel location={location} marker={track.markers[0]} markerNumber={1} />}
      ...
      <div className='fixed bottom-0 left-0 p-1 text-xs bg-white xs:ml-10 md:ml-20' style={{ zIndex: '1000' }}>
